...
<div class="page-title d-flex flex-column gap-1 mx-5 my-5  ">
    ...
    <div class="card ">
                ...
                <div class="pull-left">
                <strong>Edit User P2K3</strong>
            </div>
        </div>
      <div class="card-body">
          <form  class="lh-lg" action="{{route ('p2k3.update', $data->id)}}" enctype="multipart/form-data" method="post">
              @csrf
            {{-- <div class="ps-3 pe-5">
              ...
              <div class="ps-3 pe-5">
                ...
                <div class="col-sm-10 w-100">
                <div class="form-group label-floating is-empty is-focused">
                <input class="form-control " type="email" name="email" id="email" value="{{$data->user->email}}">
                </div>
                </div>
            </div>
              ...
              <div class="ps-3 pe-5">
                  ...
                  <div class="col-sm-10 w-100">
                      <select class="form-select fs-6 w-100" data-control="select2"
                          ...
                          <option value="Teknik Sipil" {{$data->departemen == 'Teknik Sipil' ? 'selected' : ''}}>Teknik Sipil</option>
                          <option value="Teknik Arsitektur" {{$data->departemen == 'Teknik Arsitektur' ? 'selected' : ''}}>Teknik Arsitektur</option>
                          <option value="Teknik Kimia" {{$data->departemen == 'Teknik Kimia' ? 'selected' : ''}}>Teknik Kimia</option>
                          <option value="Teknik Perencanaan Wilayah dan Kota" {{$data->departemen == 'Teknik Perencanaan Wilayah dan Kota' ? 'selected' : ''}}>Teknik Perencanaan Wilayah dan Kota</option>
                          <option value="Teknik Mesin" {{$data->departemen == 'Teknik Mesin' ? 'selected' : ''}}>Teknik Mesin</option>
                          <option value="Teknik Elektro" {{$data->departemen == 'Teknik Elektro' ? 'selected' : ''}}>Teknik Elektro</option>
                          <option value="Teknik Industri" {{$data->departemen ==  'Teknik Industri'? 'selected' : ''}}>Teknik Industri</option>
                          <option value="Teknik Lingkungan" {{$data->departemen == 'Teknik Lingkungan' ? 'selected' : ''}}>Teknik Lingkungan</option>
                          <option value="Teknik Perkapalan" {{$data->departemen == 'Teknik Perkapalan' ? 'selected' : ''}}>Teknik Perkapalan</option>
                          <option value="Teknik Perkapalan" {{$data->departemen == 'Teknik Perkapalan' ? 'selected' : ''}}>Teknik Perkapalan</option>
                          <option value="Teknik Perkapalan" {{$data->departemen == 'Teknik Perkapalan' ? 'selected' : ''}}>Teknik Perkapalan</option>
                          <option value="Teknik Komputer" {{$data->departemen == 'Teknik Komputer' ? 'selected' : ''}}>Teknik Komputer</option>
                          <option value="Teknik Komputer" {{$data->departemen == 'Teknik Komputer' ? 'selected' : ''}}>Teknik Komputer</option>
                      </select>
        ...
        height: 38px; margin : 10px 20px 30px 20px; font-size:14px; border-radius: 5px;" data-bs-toggle="modal" data-bs-target="#simpandata" onclick="showDiv()">Simpan Data</button>
                      ...
                      <a href="{{ route('p2k3.edit', $data->id) }}" type ="submit" class="btn btn-secondary text-white d-flex align-items-center justify-content-center" data-bs-toggle="modal" data-bs-target="#resetform" style="background: #868E96; margin : 10px 20px 30px 20px; width: 124.33px;
                      ...
                      <div class="modal fade" id="resetform" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                          <div class="modal-dialog modal-dialog-centered ">
                              <div class="modal-content">
                                  ...
                                  <div class="modal-body mt-5 d-flex justify-content-center align-items-center">
                                      <h2 class="mt-5 text-center" style="color: #16243D; font-size: 20px font-weight:700">keluar dari edit data?
                                          <p class="mb-0 mt-2 text-center " style="color: #DC3545; font-weight:400; font-size:14px" > data yang dimasukkan belum tersimpan </p> </h2>
                                  </div>
                                  <div class="modal-footer d-flex justify-content-center border-0">
                                      <a href="{{ route('p2k3.edit', $data->id) }}" type="button" class="btn btn-success text-white d-flex justify-content-center align-items-center text-center rounded-1" style="width:76px; height:31px; background: #29CC6A;">Ya</a>
                                      <button type="button" class="btn btn-secondary text-center d-flex align-items-center rounded-1" data-bs-dismiss="modal" style="width:76px; height:31px; ">Tidak</button>
