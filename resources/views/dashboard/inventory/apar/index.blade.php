    ...
    <div class="page-title d-flex flex-column gap-1 mx-5 my-5  ">
        ...
        <div id="kt_app_content"
            ...
            <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100 mb-5 px-5 border-bottom border-5">
                ...
                <!--end::Main wrapper-->
                <a href="{{ route('apar.tambah') }}" type="button" class="btn btn-primary btn-sm"
            ...
            <table class="table table-bordered border-secondary px-3 py-3 mb-5 shadow">
                ...
                <tbody>
                    @forelse ($inventory as $data)
                        <tr>
                            <td scope="row" class="text-center">{{ ($inventory->currentpage()-1) * $inventory ->perpage() + $loop->index + 1 }}</td>
                            <td>{{$data->name}}</td>
                            <td>{{$data->departemen->name}}</td>
                            <td>{{$data->tipe}}</td>
                            <td>{{$data->kondisi}}</td>
                            <td><a href="{{ route('apar.lihat', $data->id) }}" type="button" class="btn  btn-sm bg-warning "
                                    style="width:20px;"><i
                                        class="bi bi-eye text-dark d-flex justify-content-center align-items-center"></i></a>
                                <a href="{{ route('apar.edit', $data->id) }}" type="button" class="btn  btn-sm bg-primary"
                                    style="width:20px;"><i
                        ...
                        <div class="modal fade" id="deleteForm{{ $data->id }}" data-bs-backdrop="static" data-bs-keyboard="false"
                            ...
                            <div class="modal-dialog modal-dialog-centered ">
                                <div class="modal-content">
                                    ...
                                    <form method="POST" action="{{ route('apar.delete', $data->id) }}">
                                        @csrf
