        ...
        <div class="page-title d-flex flex-column  gap-1 mx-5 my-5  ">
            <div id="kt_app_content"
                ...
                <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100 mb-5 px-5 border-bottom border-5">
                    <!--begin::Page title-->
                    <h2>Hazard</h2>
                    <a href="{{ route('hazard.detail', $id) }}" type="button"
                ...
                <div class="page-title  gap-1 mx-5 my-5  ">
                    <form class="lh-lg" action="{{ route('hazard.update', $haz->id) }}" method="POST">
                        @csrf
                        <div class="card">
                            <div class="card-header d-flex align-items-center fs-3 fw-normal">
                                <div class="pull-left">
                                    <strong>Ubah Data Hazard</strong>
                                </div>
                            </div>
                            <div class="card-body">
                                <div class="mb-3">
                                    <label for="name" class="col-sm-2 col-form-label">Hazard</label>
                                    <div class="col-sm-10 w-100">
                                        <input type="text" class="form-control" name="name" value="{{ $haz->name }}">
                                    </div>
                                </div>

                                <div class="mb-3">
                                    <label for="departemen" class="form-label ">Pilih Lokasi:</label>
                                    <select id="aktifitas" name="aktifitas[]" class="form-select" data-control="select2" data-placeholder="Pilih Lokasi" multiple>
                                        <option value="">Pilih Lokasi</option>
                                        @foreach ($activities as $act)
                                            <option value="{{ $act->id }}" {{ in_array($act->id, explode(",", $haz->aktifitas)) ? 'selected' : '' }}>{{ $act->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="container d-flex justify-content-center">
                            <div class=" d-flex justify-content-center">
                                ...
                                <a href="{{ route('hazard.edit', ['id' => $id, 'id_haz' => $haz->id]) }}" type="submit"
