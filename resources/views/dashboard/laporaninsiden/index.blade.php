            ...
            <table class="table table-rounded table-bordered">
                ...
                <tbody>
                    @foreach ($laporaninsidens as $lap)
                        <tr>
                            <td scope="row" class="text-center">{{ ($laporaninsidens->currentpage()-1) * $laporaninsidens ->perpage() + $loop->index + 1 }}</td>
                            ...
                            <td align="center">
                                ...
                                @elseif ($lap->status == "2")
                                    @if (auth()->user()->hak_akses == "admin" || auth()->user()->hak_akses == "p2k3")
                                        <a href="{{ route('daftarinvestigasi.tambah', $lap->id) }}" class="btn btn-primary btn-sm py-2">Investigasi</a>
                                    @else
                            ...
                            <td align="center">
                                @if (auth()->user()->hak_akses == "admin" || auth()->user()->hak_akses == "p2k3")
                                    <a href="{{ route('laporan-insiden.lihat', $lap->id) }}" type="button" class="btn btn-sm btn-warning px-4"><i
                                    ...
                                    <button type="button" class="btn btn-danger btn-sm px-4" data-bs-toggle="modal" data-bs-target="#deleteForm{{ $lap->id }}"><i class="bi bi-trash pe-0"></i></button>

                                    <div class="modal fade" id="deleteForm{{ $lap->id }}" data-bs-backdrop="static" data-bs-keyboard="false"
                                    tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered ">
                                            <div class="modal-content">

                                                <form method="POST" action="{{ route('laporan-insiden.delete', $lap->id) }}">
                                                    @csrf
